export class model {
  ...
  setUser(user, unusedUserAction) {
    this.base.user.checkUser(user, ["b1", "b2", "H_init", "T_init", "u1", "u2"], unusedUserAction);
    var internal = this.internal;
    ...
    this.base.user.setUserScalar(user, "H_init", internal, 0.59999999999999998, -Infinity, Infinity, false);
  ...
  rhs(t, state, dstatedt) {
    ...
    const H = state[0];
    const T = state[1];
    dstatedt[0] = 0 + 1 * internal.bone_marrow * (internal.b1 * H * (1 - H)) - 1 * internal.bone_marrow * internal.u1 * H;
    dstatedt[1] = 0 + 1 * internal.bone_marrow * (internal.b2 * T * (1 - T - H)) - 1 * internal.bone_marrow * internal.u2 * T - 1 * internal.bone_marrow * (internal.b1 * T * H);
  }
  names() {
  ...
  updateMetadata() {
    ...
    this.metadata.internalOrder = {b1: null, b2: null, bone_marrow: null, H_init: null, initial_H: null, initial_T: null, T_init: null, u1: null, u2: null};
