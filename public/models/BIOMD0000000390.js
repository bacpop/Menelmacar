export class model {
  constructor(base, user, unusedUserAction) {
    ...
    internal.cytosol = 1;
    internal.E_RuBisCO = 3.5600000000000001;
    internal.Pic = 1.3999999999999999;
    internal.TPc = 0.20000000000000001;
  ...
  setUser(user, unusedUserAction) {
    this.base.user.checkUser(user, ["ADP_init", "ATP_init", "K", "K1", "K2", "K3", "kxk", "P_0", "PGA_init", "Pi_init", "Ru5P_init", "RuBP_init", "TP_init", "V6", "Vm"], unusedUserAction);
    var internal = this.internal;
  ...
  rhs(t, state, dstatedt) {
    ...
    dstatedt[4] = 0 + 1 * internal.chloroplast * (internal.Vm * (PGA * ATP - ADP * TP * Pi / internal.kxk) / (internal.K1 + PGA * ATP * internal.K1 / internal.K2 + ADP * TP * Pi / internal.kxk)) + 2 * internal.chloroplast * (internal.Vm * TP / (TP + internal.K)) + 1 * (internal.Vm * (TP * internal.Pic - internal.TPc * Pi) / ((TP + internal.TPc) * internal.K2 + (internal.Pic + Pi) * internal.K1 + internal.K1 * internal.K2 * (TP / internal.K1 + Pi / internal.K2) * (internal.Pic / internal.K2 + internal.TPc / internal.K1))) - 1 * internal.chloroplast * (internal.V6 * ADP * Pi / ((ADP + internal.K1) * (Pi + internal.K2)));
    ...
    dstatedt[2] = 0 + 1 * internal.chloroplast * (internal.Vm * (PGA * ATP - ADP * TP * Pi / internal.kxk) / (internal.K1 + PGA * ATP * internal.K1 / internal.K2 + ADP * TP * Pi / internal.kxk)) - 5 * internal.chloroplast * (internal.Vm * TP / (TP + internal.K)) - 1 * (internal.Vm * (TP * internal.Pic - internal.TPc * Pi) / ((TP + internal.TPc) * internal.K2 + (internal.Pic + Pi) * internal.K1 + internal.K1 * internal.K2 * (TP / internal.K1 + Pi / internal.K2) * (internal.Pic / internal.K2 + internal.TPc / internal.K1)));
    var totRuBP = 1 / 2 * (internal.P_0 - (PGA + TP + Ru5P + Pi + ATP));
    dstatedt[1] = 0 + 2 * internal.chloroplast * (internal.kxk / 2 * (internal.E_RuBisCO + totRuBP + internal.K - Math.sqrt(Math.pow((internal.E_RuBisCO + totRuBP + internal.K), (2)) - 4 * internal.E_RuBisCO * totRuBP))) - 1 * internal.chloroplast * (internal.Vm * (PGA * ATP - ADP * TP * Pi / internal.kxk) / (internal.K1 + PGA * ATP * internal.K1 / internal.K2 + ADP * TP * Pi / internal.kxk));
    dstatedt[0] = 0 - 1 * internal.chloroplast * (internal.kxk / 2 * (internal.E_RuBisCO + totRuBP + internal.K - Math.sqrt(Math.pow((internal.E_RuBisCO + totRuBP + internal.K), (2)) - 4 * internal.E_RuBisCO * totRuBP))) + 1 * internal.chloroplast * (internal.Vm * Ru5P * ATP / (internal.K1 * internal.K2 + internal.K2 * ATP + Ru5P * ATP + internal.K3 * Pi));
  }
  names() {
  ...
  updateMetadata() {
    this.metadata = {};
    var internal = this.internal;
    this.metadata.ynames = ["t", "RuBP", "PGA", "TP", "Ru5P", "Pi", "ATP", "ADP"];
    this.metadata.internalOrder = {ADP_init: null, ATP_init: null, chloroplast: null, cytosol: null, E_RuBisCO: null, initial_ADP: null, initial_ATP: null, initial_PGA: null, initial_Pi: null, initial_Ru5P: null, initial_RuBP: null, initial_TP: null, K: null, K1: null, K2: null, K3: null, kxk: null, P_0: null, PGA_init: null, Pi_init: null, Pic: null, Ru5P_init: null, RuBP_init: null, TP_init: null, TPc: null, V6: null, Vm: null};
    this.metadata.variableOrder = {RuBP: null, PGA: null, TP: null, Ru5P: null, Pi: null, ATP: null, ADP: null};
