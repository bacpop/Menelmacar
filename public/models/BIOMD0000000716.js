export class model {
  ...
  rhs(t, state, dstatedt) {
    ...
    const S_h = state[2];
    const I_a = state[3];
    dstatedt[3] = 0 + 1 * (internal.beta_bh * S_h * I_b) - 1 * internal.Human_Population * internal.d * I_a - 1 * internal.Human_Population * internal.mu_h * I_a;
    dstatedt[1] = 0 + 1 * internal.compartment * (internal.beta_b * S_b * I_b) - 1 * internal.compartment * internal.delta_b * I_b - 1 * internal.compartment * internal.mu_b * I_b;
    dstatedt[0] = 0 + 1 * internal.compartment * (internal.LAMBDA_b) - 1 * internal.compartment * internal.mu_b * S_b - 1 * internal.compartment * (internal.beta_b * S_b * I_b);
    dstatedt[2] = 0 + 1 * internal.Human_Population * (internal.LAMBDA_h) - 1 * internal.Human_Population * internal.mu_h * S_h - 1 * (internal.beta_bh * S_h * I_b);
  }
  names() {
