<template>
  <div v-if="model!==null">
    <div id="checkboxes">
      ...
      <div v-if="graph">
        <Popper>
          ...
          <template #content>
            <img src="../../public/assets/SBML_stylesheet.svg" alt="Legend" id="legend">
          </template>
        </Popper>
      </div>
    </div>
      ...
      <Popper>
        ...
        <template #content>
          <div v-for="value in Object.entries(this.parameters)" :key="value[1]">
            ...
            <input class="values" type="number" :value="value[1]"
                   @change="event => this.parameters[value[0]] = +event.target.value"/>
          </div>
    ...
    <SingleViewer v-if="!multiple && !graph" :times="times" :results_names="results_names" :results_y="results_y"
                  :log_scale="log_scale" :key="update_single"/>
    <MultipleViewer v-else-if="!graph" :times="times" :results_names="results_names" :results_y="results_y"
                    :key="update_multiple"/>
